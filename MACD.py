class Strategy():
    ...
    def __init__(self):
        ...
        self.dif_short = 12
        self.macd_period = 9
        self.dif_trace =  np.array([])
        self.UP = 1
        self.DOWN = 2


    def get_current_ma_cross(self):
        s_ma = talib.SMA(self.close_price_trace, self.dif_short)[-1]
        l_ma = talib.SMA(self.close_price_trace, self.dif_long)[-1]
        if np.isnan(s_ma) or np.isnan(l_ma):
            return None
        self.dif_trace = np.append(self.dif_trace, [float(s_ma - l_ma)])
        self.dif_trace = self.dif_trace[-self.macd_period:]
        macd = talib.SMA(self.dif_trace, self.macd_period)[-1]
        if np.isnan(macd):
            return None
        if (s_ma-l_ma) > macd:
            return self.UP
    ...
    def trade(self, information):
        ...
        self.close_price_trace = self.close_price_trace[-self.dif_long:]
